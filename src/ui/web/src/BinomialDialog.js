...
class BinomialDialog extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
        selectedVars : [],
    ...
    render() {
        const title = <div onClick={() => this.setState({ show: true})}>Binomial</div>;
        if (!this.state.show) {
    ...
    selectionsAreValid() {
      return this.state.selectedVars.length > 0;

  }

    generateSyntax() {
      const buffer = ["NPAR TEST /BINOMIAL(0.5)="];
