/* src/language/stats/crosstabs.c		-*- mode: c; buffer-read-only: t -*-

   Generated by q2c from src/language/stats/crosstabs.q.
   Do not modify!
 */
#line 1 "src/language/stats/crosstabs.q"
...
#line 77 "src/language/stats/crosstabs.c"
#include <stdlib.h>
...
#line 71 "src/language/stats/crosstabs.q"

#line 94 "src/language/stats/crosstabs.c"
#line 91 "src/language/stats/crosstabs.q"
#line 96 "src/language/stats/crosstabs.c"
struct dataset;
...
#define MAXLISTS 10
/* Array indices for WRITE subcommand. */
enum
  {
    CRS_WR_NONE = 0,
    CRS_WR_CELLS = 1,
...
struct cmd_crosstabs
  {
    ...
    /* CELLS subcommand. */
    int sbc_cells;
    int a_cells[CRS_CL_count];

    /* STATISTICS subcommand. */
    int sbc_statistics;
    int a_statistics[CRS_ST_count];
  };

/* Prototype for custom subcommands of CROSSTABS. */
static int crs_custom_tables (struct lexer *, struct dataset *, struct cmd_crosstabs *, void *);
static int crs_custom_variables (struct lexer *, struct dataset *, struct cmd_crosstabs *, void *);

/* Command parsing functions. */
static int parse_crosstabs (struct lexer *, struct dataset *, struct cmd_crosstabs *, void *);
static void free_crosstabs (struct cmd_crosstabs *);

#line 92 "src/language/stats/crosstabs.q"
#line 219 "src/language/stats/crosstabs.c"
static int
parse_crosstabs (struct lexer *lexer, struct dataset *ds, struct cmd_crosstabs *p, void *aux UNUSED)
{
  p->sbc_tables = 0;
  p->sbc_variables = 0;
  p->sbc_missing = 0;
  p->miss = CRS_TABLE;
  p->sbc_count = 0;
  p->roundwhat = CRS_CELL;
  p->roundhow = CRS_ROUND;
  p->sbc_write = 0;
  memset (p->a_write, 0, sizeof p->a_write);
  p->sbc_format = 0;
  p->val = CRS_AVALUE;
  p->indx = CRS_NOINDEX;
  p->tabl = CRS_TABLES;
  ...
  for (;;)
    {
      switch (crs_custom_tables (lexer, ds, p, aux))
        {
        case 0:
          goto lossage;
        case 1:
      ...
      if (lex_match_id (lexer, "TABLES"))
      ...
      else if (lex_match_id (lexer, "VARIABLES"))
        {
          lex_match (lexer, T_EQUALS);
          p->sbc_variables++;
          switch (crs_custom_variables (lexer, ds, p, aux))
            {
            case 0:
              goto lossage;
            case 1:
      ...
      else if (lex_match_id (lexer, "WRITE"))
        {
          lex_match (lexer, T_EQUALS);
          p->sbc_write++;
          while (lex_token (lexer) != T_SLASH && lex_token (lexer) != T_ENDCMD)
            {
              if (lex_match_id (lexer, "NONE"))
                p->a_write[CRS_WR_NONE] = 1;
              else if (lex_match_id (lexer, "CELLS"))
                p->a_write[CRS_WR_CELLS] = 1;
              else if (lex_match (lexer, T_ALL))
                p->a_write[CRS_WR_ALL] = 1;
              else
                {
                  lex_error (lexer, NULL);
                  goto lossage;
          ...
          while (lex_token (lexer) != T_SLASH && lex_token (lexer) != T_ENDCMD)
            {
              if (lex_match_id (lexer, "AVALUE"))
                ...
                p->tabl = CRS_NOTABLES;
              else if (lex_match_id (lexer, "BOX"))
                p->box = CRS_BOX;
              else if (lex_match_id (lexer, "NOBOX"))
                p->box = CRS_NOBOX;
              else if (lex_match_id (lexer, "PIVOT"))
                p->pivot = CRS_PIVOT;
              else if (lex_match_id (lexer, "NOPIVOT"))
                p->pivot = CRS_NOPIVOT;
              else
                {
                  lex_error (lexer, NULL);
          ...
          while (lex_token (lexer) != T_SLASH && lex_token (lexer) != T_ENDCMD)
            {
              if (lex_match_id (lexer, "COUNT"))
                p->a_cells[CRS_CL_COUNT] = 1;
              else if (lex_match_id (lexer, "EXPECTED"))
                p->a_cells[CRS_CL_EXPECTED] = 1;
              else if (lex_match_id (lexer, "ROW"))
                p->a_cells[CRS_CL_ROW] = 1;
              else if (lex_match_id (lexer, "COLUMN"))
                p->a_cells[CRS_CL_COLUMN] = 1;
              else if (lex_match_id (lexer, "TOTAL"))
                p->a_cells[CRS_CL_TOTAL] = 1;
              else if (lex_match_id (lexer, "RESIDUAL"))
                p->a_cells[CRS_CL_RESIDUAL] = 1;
              else if (lex_match_id (lexer, "SRESIDUAL"))
                p->a_cells[CRS_CL_SRESIDUAL] = 1;
              else if (lex_match_id (lexer, "ASRESIDUAL"))
                p->a_cells[CRS_CL_ASRESIDUAL] = 1;
              else if (lex_match (lexer, T_ALL))
                p->a_cells[CRS_CL_ALL] = 1;
              else if (lex_match_id (lexer, "NONE"))
                p->a_cells[CRS_CL_NONE] = 1;
              else
                {
                  lex_error (lexer, NULL);
                  goto lossage;
      ...
      else if (lex_match_id (lexer, "STATISTICS"))
        {
          ...
          while (lex_token (lexer) != T_SLASH && lex_token (lexer) != T_ENDCMD)
            {
              if (lex_match_id (lexer, "CHISQ"))
                p->a_statistics[CRS_ST_CHISQ] = 1;
              else if (lex_match_id (lexer, "PHI"))
                p->a_statistics[CRS_ST_PHI] = 1;
              else if (lex_match_id (lexer, "CC"))
                p->a_statistics[CRS_ST_CC] = 1;
              else if (lex_match_id (lexer, "LAMBDA"))
                p->a_statistics[CRS_ST_LAMBDA] = 1;
              else if (lex_match_id (lexer, "UC"))
                p->a_statistics[CRS_ST_UC] = 1;
              else if (lex_match_id (lexer, "NONE"))
                p->a_statistics[CRS_ST_NONE] = 1;
              else if (lex_match_id (lexer, "BTAU"))
                p->a_statistics[CRS_ST_BTAU] = 1;
              else if (lex_match_id (lexer, "CTAU"))
                p->a_statistics[CRS_ST_CTAU] = 1;
              else if (lex_match_id (lexer, "RISK"))
                p->a_statistics[CRS_ST_RISK] = 1;
              else if (lex_match_id (lexer, "GAMMA"))
                p->a_statistics[CRS_ST_GAMMA] = 1;
              else if (lex_match_id (lexer, "D"))
                p->a_statistics[CRS_ST_D] = 1;
              else if (lex_match_id (lexer, "KAPPA"))
                p->a_statistics[CRS_ST_KAPPA] = 1;
              else if (lex_match_id (lexer, "ETA"))
                p->a_statistics[CRS_ST_ETA] = 1;
              else if (lex_match_id (lexer, "CORR"))
                p->a_statistics[CRS_ST_CORR] = 1;
              else if (lex_match (lexer, T_ALL))
                p->a_statistics[CRS_ST_ALL] = 1;
              else
                {
                  lex_error (lexer, NULL);
                  goto lossage;
...
static void
free_crosstabs (struct cmd_crosstabs *p UNUSED)
{
}
#line 93 "src/language/stats/crosstabs.q"
